...
We investigate INT4 weight-only quantization as a mitigation.  Selective INT4 of reconstruction weights preserves quality ($\Delta$PPL~=~+0.051 on wikitext-2, vs.\ +6.057 for naive whole-model INT4).  However, a custom batched W4A16 Triton kernel achieves only 0.49$\times$ of cuBLAS FP16; we identify \textbf{L2 cache residency} as the root cause.  The 16~MB reconstruction weight matrix fits within H100's 50~MB L2, invalidating the HBM-bound roofline assumption that motivates quantization.  We validate this causally: scaling weight size from 8~MB to 128~MB across the L2 boundary, the INT4/FP16 gap drops from 1.9$\times$ to 1.08$\times$, with NCU confirming FP16 is DRAM-bound (83\% utilization) while INT4 is SM-bound from dequantization (79\%).  We are not aware of prior published characterization of this failure mode.
...
\begin{itemize}[leftmargin=*,topsep=0pt]
  ...
  \item \textbf{The L2 cache barrier}: roofline-predicted 3.9$\times$ INT4 speedup collapses to 0.49$\times$ because reconstruction weights are L2-resident, validated by a weight-size sweep across the L2 boundary with NCU counter analysis.  We are not aware of prior published characterization of this failure mode.
...
\textbf{NCU profiling.}  Collected with \texttt{ncu --set full --profile-from-start no} (kernel replay); overhead excluded from bandwidth runs.
...
We validate this directly by scaling the weight matrix across the L2 boundary.  Holding $H{=}128$ and $d_\text{nope}{=}128$ fixed, we sweep $d_\text{lora}$ from 256 to 4096, producing FP16 weight sizes from 8~MB to 128~MB.  Figure~\ref{fig:l2sweep} shows the result: the INT4/FP16 time ratio drops from 1.91$\times$ at 8~MB to 1.08$\times$ at 128~MB, with a clear knee at 40--48~MB as weights begin to exceed L2 capacity.  At MLA's operating point (16~MB), INT4 is 1.86$\times$ slower; at 128~MB (well past L2), the gap nearly closes.
...
\end{groupplot}
\end{tikzpicture}
\caption{Cache-residency intervention at fixed MLA reconstruction shape (BS=1, normalized to FP16-warm).  H100: FP16 rises from 1.00 to 1.34 under eviction, while INT4 rises from 1.85 to 1.62 relative to FP16-warm.  A100 shows stronger asymmetry: FP16 rises from 1.00 to 1.22 while INT4 rises from 1.93 to 1.70.  FP16 is 2--3$\times$ more sensitive to forced eviction than INT4, directly supporting L2-residency dependence of FP16 and dequantization-limited INT4 behavior.}
\label{fig:cache_intervention}
\end{figure}

Figure~\ref{fig:cache_intervention} shows this intervention on both H100 and A100.  FP16 latency increases by 34\% under forced eviction on H100 and 22\% on A100.  INT4 latency increases by 17\% and 8\%, respectively, i.e., roughly 2--3$\times$ less sensitive to cache state.  This asymmetry directly supports the mechanism: FP16's advantage at the 16\,MB operating point depends on L2 residency, while INT4 remains limited by dequantization-driven SM saturation regardless of cache state.  The larger asymmetry on A100 (2.9$\times$ vs.\ 2.0$\times$) is consistent with A100's lower HBM bandwidth, which increases the latency penalty of cache eviction for the memory-bound FP16 path while leaving the compute-bound INT4 path less affected.  NCU counter attribution per tensor allocation is not available in current tooling; the timing-based differential is therefore our primary causal evidence.
...
\end{axis}
\end{tikzpicture}
\caption{Weight-size scaling across the L2 boundary on H100 and A100.  INT4/FP16 time ratio vs.\ FP16 weight size.  Both GPUs show the same knee behavior near their L2 capacities (A100: 40\,MB, H100: 50\,MB): below the knee, INT4 is much slower; above the knee, the ratio moves toward parity as FP16 falls back to HBM.  At MLA's 16\,MB operating point (arrow), both platforms remain in the L2-resident regime.}
\label{fig:l2sweep}
\end{figure}

NCU profiling across the same sweep confirms the mechanism.  On H100, the FP16 cuBLAS kernel (\texttt{nvjet}, TMA-based) is DRAM-bound: DRAM utilization scales from 35\% at 8~MB to 83\% at 128~MB, while SM utilization stays below 15\%.  The INT4 Triton kernel is the opposite: SM utilization scales from 33\% to 79\% (dequantization dominates), while DRAM utilization stays below 23\%.  On A100, we observe the same direction: FP16 DRAM utilization rises from 43\% to 66\%, while INT4 SM utilization rises from 51\% to 75\% across the sweep.  INT4 reads exactly 4$\times$ fewer DRAM bytes as expected, but converting that bandwidth savings into latency reduction requires the kernel to not be compute-bound, which it is.
...
\textbf{When does INT4 help?}  The L2 cache barrier is not absolute.  In production serving, concurrent operations (FFN GEMMs, attention across multiple layers, multi-tenant request batching) contend for L2 capacity and may evict reconstruction weights back to HBM.  INT4 reconstruction gains are therefore \textbf{deployment-dependent}: negligible in isolated benchmarks, but potentially significant under L2 pressure from high-throughput serving.  Fusing reconstruction across multiple layers to exceed L2 capacity, or deploying on GPUs with smaller L2 caches, could also restore the predicted benefit.
