...
From these measurements we construct a \textbf{measured cache-aware roofline} (capacity-gated bandwidth ceiling at the effective L2 capacity, an explicit per-launch fixed cost, and fitted dequantization in-core ceilings) that predicts FP16 and INT4 latency to 11--16\% MAPE, and we confirm its central prediction constructively: a \textbf{W8A8 kernel using INT8 tensor-core MMA} --- which removes dequantization from the inner loop --- beats cuBLAS FP16 by 1.4--1.5$\times$ at decode batch sizes exactly when weights exceed the effective L2 capacity, and loses (0.70$\times$) when they are L2-served.  The same regime structure governs mixed-precision MoE kernels (FlagGems), where we locate and fix a host-side dequantization defect; the corrected weight-only kernel sustains 1.7$\times$ (low token counts) to 2.7--3.0$\times$ (T$\geq$512) over bf16 with no measured crossover through T=2048.
