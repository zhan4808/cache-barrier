...
\begin{itemize}[leftmargin=*,topsep=0pt]
  ...
  \item \textbf{A measured cache-aware roofline}: capacity-gated bandwidth ceiling (effective $C_\text{eff}{\approx}36$~MB), explicit per-launch fixed cost (2.8~$\mu$s graph / 15.4~$\mu$s eager), and fitted dequant in-core ceilings; predicts FP16/INT4 latency to 11--16\% MAPE (Section~\ref{sec:carm}).
  \item \textbf{Constructive confirmation (W8A8)}: an INT8 tensor-core MMA kernel with no inner-loop dequantization beats cuBLAS FP16 by 1.4--1.5$\times$ at bs=1 exactly above $C_\text{eff}$ and loses below it --- the first quantized win in this regime, flipping at the measured (not nominal) L2 boundary (Section~\ref{sec:w8a8}).  The same regime structure governs mixed-precision MoE, where we fix a FlagGems W8A16 defect; the corrected kernel wins at every measured token count, 2.7--3.0$\times$ at T$\geq$512 (Section~\ref{sec:moe}).
...
The INT4 quantization results illustrate a subtlety in MLA optimization.  \textbf{Quality is safe}: selective INT4 adds only +0.051 perplexity.  \textbf{But speedup is not automatic}: our custom Triton W4A16 kernel achieves 0.49$\times$ of cuBLAS FP16 through two stacked mechanisms --- ${\sim}75\%$ of the 16~MB reconstruction weights are L2-served in steady state (effective capacity ${\sim}36$~MB, effective bandwidth 3.5--4.6~TB/s), removing most of the HBM savings INT4 targets; and the dequantization-bound kernel forfeits the remainder, losing to FP16 even when residency is experimentally removed.  The gap between roofline-predicted (3.9$\times$) and realized performance is specific to small, (partially) L2-resident weight matrices, a regime common in MLA but absent from standard LLM linear layers.

The measured cache-aware roofline turns these failures into a usable decision procedure, and the W8A8 result closes the loop: with dequantization removed from the inner loop (INT8 tensor-core MMA, scales on the accumulator), quantization wins 1.4--1.5$\times$ precisely where the model says it should --- weight-byte-bound decode above the effective L2 capacity --- and nowhere else.  The deployment rule for reconstruction-style GEMMs is three-way and fully measurable on the target GPU: serve from L2 in FP16; quantize with INT8 MMA when HBM-bound on weight bytes; stay in FP16 when compute-bound.  Fixing residency alone, or kernel efficiency alone, is provably insufficient; the regime decides.
