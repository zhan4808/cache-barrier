...
\begin{itemize}[leftmargin=*,topsep=0pt]
  ...
  \item \textbf{Cross-architecture transfer} (Section~\ref{sec:transfer}): a portable harness measures the parameters on any CUDA-semantics backend.  Fit on H100 and applied to A100: 23--29\% MAPE zero-shot on fp16, 19.9\% on W4A16 after a two-point kernel calibration; naive peak-ratio scaling of kernel terms fails (40.6\%), establishing that kernel terms must be measured, not extrapolated.
...
\subsection{Hardware}
Primary measurements use a single NVIDIA H100 80GB SXM5 (HBM3).  Cross-hardware validation uses a single NVIDIA A100 80GB SXM4 (HBM2e).  Reference peaks for H100: 3.35~TB/s HBM bandwidth, 989.4~TFLOPS BF16 tensor core, 132 SMs, 50~MB L2 cache.  Reference peaks for A100: 2.0~TB/s HBM bandwidth, 312~TFLOPS BF16 tensor core, 108 SMs, 40~MB L2 cache.  The 2026-08 measurements (Sections~\ref{sec:gate}--\ref{sec:dispatch}) are clock-locked at 1755~MHz (\texttt{nvidia-smi -lgc}); the earlier case-study measurements were not, and Section~\ref{sec:repro} quantifies the resulting variance.
...
\begin{figure}[t]
  ...
  \caption{\textbf{Transfer to A100} (bs=1).  Left: fp16 latency predicted zero-shot from the model form plus A100 hardware constants --- nothing fitted to A100 kernel measurements.  Right: the W4A16 Triton kernel after a two-point calibration of $(r_\text{dequant},\ \text{fixed cost})$ on the target; the dotted line scales H100 kernel terms by peak ratio and fails.  Data: \texttt{profiling/portable/results\_transfer\_a100.json}.}
  \label{fig:transfer}
\end{figure}
...
Applied across architectures (Figure~\ref{fig:transfer}): with A100 hardware constants and zero A100 kernel fitting, fp16 predictions land at 28.8\%/22.8\% MAPE (below/above the gate) --- an upper bound, since the available A100 constants are estimates rather than harness measurements.  Kernel terms do not transfer by scaling: extrapolating H100's $r_\text{dequant}$ by peak ratio yields 40.6\% MAPE and is structurally blind to the A100 kernel's measured 58.7~$\mu$s fixed cost (${\sim}6~\mu$s on H100 at the same shape).  A two-point calibration on the target --- the per-kernel microbenchmark the harness automates --- predicts the remaining 22 operating points to 19.9\%.  The division of labor is the finding: \emph{hardware terms transfer as cheap measurements; kernel terms are implementation properties that must be measured per architecture, and cannot be extrapolated}.
...
\begin{itemize}[leftmargin=*,topsep=0pt]
  ...
  \item The A100 transfer numbers use estimated (not harness-measured) A100 hardware constants and an eager-timed 2026-06 target dataset; they are upper bounds pending a harness run on real A100 hardware.
